/*!
 * puxi.js - v0.0.4
 * Compiled Tue, 31 Mar 2020 18:27:05 UTC
 *
...
class ScrollManager extends EventManager {
    constructor(target, preventDefault = true) {
        ...
        this.onMouseScrollImpl = (e) => {
            ...
            this.scale = Math.min(Math.max(0, this.scale), 8);
            


            var scrollBarPercent = this.scale*73 

            target.scrollBars[0].handle.contentContainer.y = scrollBarPercent

            target.forcePctPosition('y', -this.scale)
        ...
        this.onHoverImpl = (e) => {
            ...
            if (!this.bound) {
                document.addEventListener('mousewheel', onMouseScrollImpl, {passive: false});
                document.addEventListener('DOMMouseScroll', onMouseScrollImpl, {passive: false});
                this.bound = true;
            }
        };
        // eslint-disable-next-line @typescript-eslint/no-unused-vars
...
class ScrollBar extends Slider {
    ...
    initialize() {
        ...
        this.on('changing', () => {
            ...
            //JAMES
            var maskedHeight = this.scrollingContainer.height
            var fullHeight = this.scrollingContainer.contentContainer._bounds.maxY
            let multiplier = 8
            if(fullHeight < 1000) {
                multiplier = 2
            }
            //console.log(maskedHeight, fullHeight)
            var scrollingOffset = (fullHeight/maskedHeight)*multiplier;
            //console.log((this.percentValue*-scrollingOffset))

            this.scrollingContainer.forcePctPosition(this.orientation === Slider.HORIZONTAL ? 'x' : 'y', (this.percentValue*-scrollingOffset));
        });
        this.on('change', () => {
...
class ScrollWidget extends InteractiveGroup {
    ...
    constructor(options = {}) {
        ...
        this.forcePctPosition = (direction, pct) => {
            ...
            if(this.innerContainer.contentContainer.children.length >= 1) {
                var boxHeight = this.innerContainer
                var textHeight = this.innerContainer.contentContainer.children[0]

                const opacity = this.insetContainer.children[0].children[2]
        ...
        this.updateScrollVisibility = (size) => {
            if(this.innerContainer.contentContainer.children.length >= 1) {
                var boxHeight = this.innerContainer.height
                var textHeight = this.innerContainer.contentContainer.children[0].height
                if(textHeight < boxHeight)  {
                    this.insetContainer.children[0].children[2].alpha = 0
                } else {
                    this.insetContainer.children[0].children[2].alpha = 1
                }
            }
        }
        ...
        this.updateDirection = (direction, delta) => {
            ...
            if (!this.scrolling && Math.round(scrollVelocity[direction]) !== 0) {
                ...
                scrollVelocity[direction] = Helpers.Lerp(scrollVelocity[direction], 0, (5 + 2.5 / Math.max(this.softness, 0.5)) * delta);
